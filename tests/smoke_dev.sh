#!/usr/bin/env bash
# tests/smoke_dev.sh
# Smoke tests for the dev environment.
...
# --------------------------------------------------------------------------
# 7. Deploy role 'github-deploy-dev' exists
# --------------------------------------------------------------------------
ROLE_ARN=$(aws iam get-role --role-name "github-deploy-dev" \
  --query "Role.Arn" --output text 2>/dev/null || echo "NONE")
if [ "${ROLE_ARN}" != "NONE" ] && [ -n "${ROLE_ARN}" ]; then
  pass "Deploy role 'github-deploy-dev' exists (${ROLE_ARN})"
else
  fail "Deploy role 'github-deploy-dev' not found"
fi
